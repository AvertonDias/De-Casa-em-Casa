...
export const onTerritoryUpdateForHistory = functions.firestore
    ...
    .onUpdate(async (change, context) => {
        ...
        // Se o trabalho começou (foi de 0 para > 0)
        ...
        // Se o trabalho foi concluído (de pendente > 0 para pendente = 0)
...
export const handleUserPresence = functions.database.ref('/status/{uid}')
    .onWrite(async (change, context) => {
        ...
        try {
          ...
          // Ignora erros 'not-found' se o documento do usuário foi deletado
        ...
        const congregationRef = db.doc(`congregations/${congregationId}`);
        const statusRef = change.after.ref.parent; // Referência para /status
  ...
  .onRun(async (context) => {
    ...
    try {
      await client.exportDocuments({
        ...
        collectionIds: [], // Exporta todas as coleções
      });
