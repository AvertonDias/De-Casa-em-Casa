...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Regra 1: Admin não pode se autoexcluir.
    ...
    // Regra 2: Usuário normal só pode se autoexcluir.
    if (!isCallingUserAdmin && callingUserUid !== userIdToDelete) {
      return res.status(403).json({ error: "Você não tem permissão para excluir outros usuários." });
    }

    // A verificação de senha para autoexclusão é feita no frontend ou deveria ser re-autenticada aqui.
    // Por simplicidade, confiamos no token verificado para autorizar a ação.
