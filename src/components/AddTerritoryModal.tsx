...
import { X, FileImage, Loader, HelpCircle } from 'lucide-react';
        ...
        <DialogContent className="sm:max-w-md" onOpenAutoFocus={(e) => e.preventDefault()}>
            ...
            <div className="space-y-4 py-4 px-1">
              <div className="flex items-center gap-4">
                <div className="w-28"><label className="block text-sm mb-1 font-medium">Número</label><Input ref={numberInputRef} value={number} onChange={(e) => setNumber(e.target.value)} placeholder="Ex: 12"/></div>
                <div className="flex-grow"><label className="block text-sm mb-1 font-medium">Nome</label><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex: Centro Comercial"/></div>
              </div>
              <div><label className="block text-sm mb-1 font-medium">Observações (Opcional)</label><Textarea value={description} onChange={(e) => setDescription(e.target.value)} rows={2} placeholder="Ex: Território da prefeitura..."></Textarea></div>
