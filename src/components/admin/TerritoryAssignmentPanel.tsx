...
export default function TerritoryAssignmentPanel() {
  ...
  const handleSaveAssignment = async (territoryId: string, assignedUser: { uid: string; name: string }, assignmentDate: string, dueDate: string) => {
    ...
    const campaignName = congregation?.activeCampaign?.title || null;
    ...
    try {
        await runTransaction(db, async (transaction) => {
            ...
            // Apenas inclui se não for nulo/undefined para evitar erros no Firestore
            if (campaignName) {
                assignment.campaignName = campaignName;
            }
            ...
            if (isReassignment) {
                ...
                // Se a designação anterior tinha campanha, mantém no histórico
                if (data.assignment?.campaignName) {
                    historyLog.campaignName = data.assignment.campaignName;
                }
        ...
        logEvent(
            ...
            { territoryId, assignedTo: assignedUser.name, isReassignment, campaignName: campaignName || undefined }
        );
        ...
        if (!assignedUserId.startsWith('custom_')) {
            try {
                ...
                // 1. Salvar notificação no Firestore do usuário (agora permitido pelas regras do Firestore)
                const userNotifRef = collection(db, `users/${assignedUserId}/notifications`);
  ...
  const handleConfirmReturn = async (territoryId: string, returnDate: string) => {
    ...
    if (campaignName) {
        historyLog.campaignName = campaignName;
    }
    ...
    try {
        ...
        logEvent(
            ...
            { territoryId, returnedBy: territoryToReturn.assignment.name, campaignName: campaignName || undefined }
        );
  ...
  const filteredTerritories = territories.filter(t => {
      ...
      let isOverdue = false;
      if (t.status === 'designado' && t.assignment?.dueDate) isOverdue = isTerritoryOverdue(t.assignment.dueDate);
        ...
        <div className="border border-border/40 rounded-xl overflow-hidden shadow-sm">
          <Accordion type="single" collapsible className="w-full">
            {filteredTerritories.map(t => {
                ...
                const isOverdue = isDesignado && t.assignment && isTerritoryOverdue(t.assignment.dueDate);
                return (
