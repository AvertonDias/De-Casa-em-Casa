  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      if (user.role === 'Administrador') {
        const cleanNumber = congregationNumber.trim();
        const cleanOriginalNumber = (originalData.number || '').trim();

        if (cleanNumber && cleanNumber !== cleanOriginalNumber) {
          const congQuery = query(collection(db, "congregations"), where("number", "==", cleanNumber));
          const congSnap = await getDocs(congQuery);
          const otherDocs = congSnap.docs.filter(d => d.id !== user.congregationId);
          if (otherDocs.length > 0) {
            const msg = "Este número de congregação já está em uso por outra congregação.";
            setError(msg);
            toast({
