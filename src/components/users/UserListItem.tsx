...
import { MoreVertical, Check, Trash2, XCircle, Edit, User, Clock } from 'lucide-react';
      ...
      <div className="flex items-center flex-1 min-w-0">
        ...
        <div className="min-w-0 space-y-0.5">
            ...
            <div className="flex items-center gap-1.5 text-xs font-medium text-muted-foreground">
              <Clock size={12} />
              <span>
                {isOnline ? 'Ativo agora' : (user.lastSeen?.toDate ? `Visto ${formatDistanceToNow(user.lastSeen.toDate(), { addSuffix: true, locale: ptBR })}` : 'Desconectado')}
              </span>
            </div>
        </div>
      </div>
