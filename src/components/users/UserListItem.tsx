...
export const UserListItem = ({
  ...
  const canShowMenu =
    ...
    (isAdmin || (isDirigente && user.status === "pendente"));
  ...
  return (
    <li
      className={`p-4 flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b dark:border-gray-700 ${
        user.status === "pendente" ? "bg-yellow-500/10" : ""
      }`}
    >
      {/* Dados do usuário */}
        ...
        <div className="min-w-0">
          <p className="font-semibold text-gray-900 dark:text-white truncate">
            ...
            {user.uid === currentUser.uid && (
              <span className="text-purple-400 font-normal ml-2">(Você)</span>
            )}
          </p>
          <p
            className={`text-sm ${
          ...
          >
            {isOnline
              ...
              ? `Visto ${formatDistanceToNow(user.lastSeen.toDate(), {
                  addSuffix: true,
                  locale: ptBR,
                })}`
              : "Offline"}
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <span
          className={`px-3 py-1 text-xs font-medium rounded-full ${getRoleClass(
            user.role
          )}`}
        >
          {user.role}
                    ...
                    <>
                      ...
                      <Menu.Item>
                        {({ active }: { active: boolean }) => (
                          <button
                            onClick={() =>
                              onUpdate(user.uid, { status: "rejeitado" })
                            }
                            className={`${
                              active
