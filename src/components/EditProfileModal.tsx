...
export function EditProfileModal({ isOpen, onOpenChange }: { isOpen: boolean, onOpenChange: (isOpen: boolean) => void }) {
  ...
  const handleSaveChanges = async (e: React.FormEvent) => {
    ...
    if (whatsapp.length < 15) {
        setError("Por favor, preencha o número de WhatsApp completo.");
        setLoading(false);
        return;
    }
  ...
  const confirmSelfDelete = async () => {
    ...
    try {
        ...
        onOpenChange(false);
         ...
         } else if (error.code?.includes('auth/user-mismatch')) {
             setError(`A conta Google selecionada é diferente da sua conta logada (${auth.currentUser?.email}). Por favor, selecione a mesma conta (${auth.currentUser?.email}) na janela da Google para confirmar a exclusão.`);
  ...
  const isSaveDisabled = loading || whatsapp.length < 15 || !name.trim() || !hasChanges;
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      ...
      >
          ...
          {user?.email && (
            <div className="mt-2 flex flex-col gap-1">
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    {user?.acceptedLGPD && (
                        <span className="text-[10px] bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded-full font-bold flex items-center gap-1 uppercase">
        ...
        <div className="px-6 space-y-4">
          <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-xs leading-relaxed text-emerald-600 dark:text-emerald-400 flex items-start gap-2">
            ...
            <span>
              ...
              <button 
                ...
                className="underline font-bold hover:text-emerald-700 dark:hover:text-emerald-300"
              >
                Termos de Uso
              </button>{' '}
              e{' '}
              ...
              {user?.acceptedLGPDAt ? (
                <>aceitos em: <b>
                  {(() => {
                    try {
                      const dateObj = user.acceptedLGPDAt && typeof user.acceptedLGPDAt.toDate === 'function' 
                        ? user.acceptedLGPDAt.toDate() 
                        : new Date(user.acceptedLGPDAt as any);
                      return !isNaN(dateObj.getTime()) ? dateObj.toLocaleDateString('pt-BR') : new Date().toLocaleDateString('pt-BR');
                    } catch (e) {
          ...
          <form id="edit-profile-form" onSubmit={handleSaveChanges} className="space-y-4">
            <div>
              ...
              <div className="pt-3 border-t border-red-500/30">
                ...
                {!showDeleteConfirm && (
                  <>
                    <p className="text-xs text-muted-foreground mt-1">A exclusão da sua conta é permanente, apaga todos os seus dados e retira seu acesso ao sistema.</p>
                    <Button
                      type="button"
                      variant="destructive"
                ...
                {showDeleteConfirm && (
                  <div className="mt-2 p-3 bg-destructive/10 rounded-lg border border-destructive/20 space-y-3">
                    <p className="text-xs font-semibold leading-relaxed">
                      {isGoogleUser 
                          ? "Para confirmar a exclusão permanente, autentique-se novamente com sua conta Google."
                          : "Para confirmar a exclusão permanente da conta, digite sua senha atual."
                      }
                    </p>
                    ...
                    <div className="flex gap-2 justify-end">
                        <Button type="button" variant="secondary" size="sm" onClick={() => setShowDeleteConfirm(false)}>Cancelar</Button>
                        <Button
                          type="button"
                          variant="destructive"
                          size="sm"
          ...
          {error && <p className="text-destructive font-bold text-xs pb-2 text-center">{error}</p>}
        </div>
      ...
      <LgpdModal isOpen={isLgpdDetailOpen} onOpenChange={setIsLgpdDetailOpen} type={lgpdModalType} />
    </Dialog>
  );
}