...
export default function EditTerritoryModal({ territory, isOpen, onClose, onSave, onReset, onDelete }: EditTerritoryModalProps) {
  ...
  // cardUrl guarda o texto da imagem que veio do Firestore ou o novo a ser salvo
  const [cardUrl, setCardUrl] = useState(''); 
  // previewUrl guarda a URL local temporária para a NOVA imagem selecionada
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);

  const [isProcessing, setIsProcessing] = useState(false);
  const [error, setError] = useState<string | null>(null);

  // Preenche o modal quando ele é aberto
  useEffect(() => {
    if (isOpen && territory) {
      ...
      setCardUrl(territory.cardUrl || ''); // Pega o texto da imagem do Firestore
      setError(null);
      
      // Limpa a pré-visualização ao abrir
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        // Guarda tanto a nova URL de preview quanto o dado a ser salvo
        setPreviewUrl(newCardUrl); 
        setCardUrl(newCardUrl); // Atualiza o dado a ser salvo
      };
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex justify-center items-center z-50 p-4">
      <div className="bg-card text-card-foreground p-6 rounded-lg shadow-xl w-full max-w-md relative overflow-y-auto max-h-[90vh]">
