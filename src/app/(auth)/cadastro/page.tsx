  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.code === 'auth/email-already-in-use') { 
        ...
        setError("A senha escolhida é muito fraca. Tente uma mais complexa.");
      } else { 
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/account-exists-with-different-credential') {
        setError("Já existe uma conta com este e-mail. Tente fazer login com sua senha.");
        ...
        <div className="w-full max-w-sm p-8 space-y-6 bg-card text-card-foreground rounded-xl shadow-lg border border-border/50">
          ...
          <form onSubmit={handleSignUp} className="space-y-4">
              <div className="space-y-3">
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Nome Completo" required className="w-full px-4 py-2 bg-background border border-input rounded-md focus:ring-2 focus:ring-primary outline-none" autoComplete="name" />
                <input type="email" value={email} onChange={handleEmailChange} placeholder="Seu melhor e-mail" required className="w-full px-4 py-2 bg-background border border-input rounded-md focus:ring-2 focus:ring-primary outline-none" autoComplete="email" />
                
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Criar Senha (mín. 6 caracteres)" required className="w-full px-4 py-2 bg-background border border-input rounded-md pr-10 focus:ring-2 focus:ring-primary outline-none" autoComplete="new-password" />
                ...
                <div className="relative">
                  <input type={showConfirmPassword ? "text" : "password"} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} placeholder="Repetir Senha" required className="w-full px-4 py-2 bg-background border border-input rounded-md pr-10 focus:ring-2 focus:ring-primary outline-none" autoComplete="new-password" />
                ...
                <div className="pt-2">
                  <p className="text-[10px] text-muted-foreground uppercase font-bold mb-1 ml-1">Dados da Congregação</p>
                  <input type="tel" inputMode="numeric" value={congregationNumber} onChange={(e) => setCongregationNumber(e.target.value.replace(/\D/g, ''))} placeholder="Número da sua Congregação" required className="w-full px-4 py-2 bg-background border border-input rounded-md focus:ring-2 focus:ring-primary outline-none" />
                </div>

                <div className="pt-2">
                  <p className="text-[10px] text-muted-foreground uppercase font-bold mb-1 ml-1">Contato WhatsApp</p>
                  <input type="tel" value={whatsapp} onChange={handleWhatsappChange(setWhatsapp)} placeholder="Seu WhatsApp" required className="w-full px-4 py-2 bg-background border border-input rounded-md focus:ring-2 focus:ring-primary outline-none" autoComplete="tel" />
                  <input type="tel" value={confirmWhatsapp} onChange={handleWhatsappChange(setConfirmWhatsapp)} placeholder="Confirmar seu WhatsApp" required className="w-full px-4 py-2 bg-background border border-input rounded-md mt-2 focus:ring-2 focus:ring-primary outline-none" autoComplete="tel" />
                </div>
              </div>

              <button type="submit" disabled={loading || googleLoading || !name || !email || whatsapp.length < 15} className="w-full px-4 py-2.5 font-bold text-primary-foreground bg-primary rounded-md hover:bg-primary/90 disabled:opacity-50 transition-all shadow-md mt-4">
                {loading ? <><Loader className="animate-spin inline mr-2" size={18}/> Enviando...</> : 'Solicitar Acesso'}
              </button>
          </form>
