...
const OfflineDownloader = ({ territoryId, isManagerView }: { territoryId: string, isManagerView: boolean }) => {
    ...
    return (
      <button
        ...
        title={isDownloaded ? `Baixado em ${format(downloadState.downloadedAt!, 'dd/MM HH:mm')}` : 'Baixar para offline'}
      >
        {isDownloading 
          ? <Loader className="animate-spin" size={18} /> 
          : <Download size={18} className={isDownloaded ? 'text-green-400' : 'text-muted-foreground'} />
        }
      </button>
    );
};
