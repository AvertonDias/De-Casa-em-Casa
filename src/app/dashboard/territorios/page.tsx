...
const TerritoryRowManager = ({ territory }: { territory: Territory }) => {
  ...
  const dateObj = dueDate && typeof (dueDate as any).toDate === 'function' ? (dueDate as any).toDate() : (dueDate ? new Date(dueDate as any) : new Date());
  
  const isDesignado = territory.status === 'designado' && territory.assignment;
  const isOverdue = isDesignado ? isTerritoryOverdue(dueDate) : false;
        ...
        {territory.type !== 'rural' && (
          <div className="pt-2 mt-4">
            <div className="grid grid-cols-4 gap-1 sm:gap-4 text-center">
                <div><p className="text-xs sm:text-sm text-muted-foreground">Total</p><p className="font-bold text-lg sm:text-2xl">{totalCasas}</p></div>
                <div><p className="text-xs sm:text-sm text-muted-foreground">Feitas</p><p className="font-bold text-lg sm:text-2xl text-green-400">{casasFeitas}</p></div>
                <div><p className="text-xs sm:text-sm text-muted-foreground">Pendentes</p><p className="font-bold text-lg sm:text-2xl text-yellow-400">{totalCasas - casasFeitas}</p></div>
                <div><p className="text-xs sm:text-sm text-muted-foreground">Progresso</p><p className="font-bold text-lg sm:text-2xl text-blue-400">{progresso}%</p></div>
            </div>
    ...
    if (statusFilter !== 'all') {
      filtered = filtered.filter(t => {
        ...
        if (t.status === 'designado' && t.assignment) {
            const isOverdue = isTerritoryOverdue(t.assignment.dueDate);
