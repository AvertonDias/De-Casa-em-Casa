...
export default function QuadraDetailPage() {
    ...
    if (highlightedHouseId) {
      requestAnimationFrame(() => {
        ...
        if (element) {
          ...
          element.classList.add('bg-purple-300', 'dark:bg-purple-800/60');
          setTimeout(() => {
            element.classList.remove('bg-purple-300', 'dark:bg-purple-800/60');
          }, 1500); 
        }
      });
  ...
  const handleConfirmStatusChange = () => {
    if (!statusAction || !user?.congregationId) return;
  ...
  const executeDelete = async () => {
    if (!casaToDelete || !user?.congregationId) return;
