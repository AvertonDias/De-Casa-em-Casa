...
import { Home, Map, Users, Settings, LogOut, Menu, X, Sun, Moon, Trees, Download, Laptop, Share2 } from 'lucide-react';
        ...
        <nav className="flex-1">
          <ul className="space-y-1">
            {filteredNavLinks.map((link) => {
              ...
              return (
                <li key={link.name}>
                  <Link href={link.href} onClick={onClose} className={cn('flex items-center justify-between text-md p-3 rounded-lg mb-2 transition-colors', isActive ? 'bg-purple-600 text-white font-semibold' : 'text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-purple-600/20')}>
...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  if (loading) {
    return (
      <div className="flex h-screen w-full items-center justify-center bg-gray-100 dark:bg-[#1E1B29]">
  ...
  // Se não estiver carregando e não houver usuário, o UserContext redireciona.
  // Retornar null evita que a tela de login pisque para usuários autenticados.
  return null;
}
