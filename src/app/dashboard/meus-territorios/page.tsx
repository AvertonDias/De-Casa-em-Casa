...
function MyTerritoriesPage() {
  ...
  const handleReturnTerritory = async () => {
    ...
    // Construção segura do log de histórico para evitar undefined
    ...
    // Só adiciona se existir, evitando erro de 'undefined' no Firestore
    if (territoryToReturn.assignment.campaignName) {
        ...
        {sortedTerritories.length > 0 ? (
          <div className="space-y-4">
            {sortedTerritories.map(t => {
              const isOverdue = t.assignment && isTerritoryOverdue(t.assignment.dueDate);
              return (
