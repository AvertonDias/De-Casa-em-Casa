...
import { Map, PlusCircle, Search, Link as LinkIcon, Loader, Inbox } from 'lucide-react';
...
export default function RuralPage() {
  ...
  const handleAddGlobalLink = async () => {
    if (!user?.congregationId) return;
    const description = prompt("Digite a descrição para o link (Ex: Mapa de Estradas Vicinais):");
    if (!description || !description.trim()) return;
    
    const url = prompt("Agora, cole a URL completa do link (iniciando com https://):");
    if (!url || !url.trim().startsWith('http')) {
        alert("URL inválida. Certifique-se de que ela começa com http:// ou https://");
        return;
    }

    const newLink: RuralLink = { id: crypto.randomUUID(), description, url };
    const congRef = doc(db, 'congregations', user.congregationId);
    await updateDoc(congRef, {
      globalRuralLinks: arrayUnion(newLink)
    });
  };
      ...
      {user?.role === 'Administrador' && (
        <div className="bg-card p-4 rounded-lg mb-6">
          <div className="flex justify-between items-center">
            <h2 className="font-semibold flex items-center"><LinkIcon size={16} className="mr-2"/> Links Gerais</h2>
            <button onClick={handleAddGlobalLink} className="text-sm text-primary hover:underline font-semibold">+ Adicionar Link</button>
          </div>
          <div className="mt-2 text-sm space-y-1">
            {congregation?.globalRuralLinks && congregation.globalRuralLinks.length > 0 ? (
                congregation.globalRuralLinks.map(link => (
                    <a href={link.url} key={link.id} target="_blank" rel="noopener noreferrer" className="block text-blue-400 hover:underline">{link.description}</a>
                ))
            ) : (
