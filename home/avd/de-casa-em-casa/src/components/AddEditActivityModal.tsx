...
const toDate = (dateString: string) => {
    ...
    return new Date(year, month - 1, day, 12, 0, 0); // Adiciona meio-dia para evitar problemas de fuso
};
  ...
  useEffect(() => {
    if (isOpen) {
      if (isEditing && activityToEdit) {
        ...
        const originalDescription = activityToEdit.description || '';
        setNotes(cleanDescription(originalDescription));
      } else {
